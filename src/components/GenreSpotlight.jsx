...
// Displays a spotlight grid of movies for a given genre
const GenreSpotlight = ({ movies, loading, error, onMovieClick }) => {
  if (loading) return <LoadingDots />; // Show loading animation
  if (error)
    return <div className="text-red-500">Error loading {genreName} movies</div>; // Display error message
      ...
      {movies.map((movie) => (
        <div
          ...
          onClick={() =>
            onMovieClick({
              title: movie.title,
              year: movie.release_date?.split("-")[0], // Extract year for modal context
            })
          }
